...
class PlaintextDataset(BaseDataset):
    ...
    def load_data(self):
        """
        entry function of plaindataset.
        """
    ...
    def __load_data_from_memory(self):
        """
        load data from memory, and this method will convert data
        package object to PlainDataset object.
        """
    ...
    def __load_data_from_path(self, data_path=None):
        """
        This method will load data from path, Given data path value from this method
        or from class.
        """
        ...
        if self.__type_doc == "csv":
            try:
                data, target, weight, feature_names, target_names, weight_names = self.__load_csv()
            except IOError:
            ...
            data, target, weight, feature_names, target_names, weight_names = self.__load_libsvm()
        elif self.__type_doc == 'txt':
            data, target, weight, feature_names, target_names, weight_names = self.__load_txt()
        else:
    ...
    def __set_proportion(self):
        """
        This method will get proportion of each label class,
        and this value will be added to self._bunch object.
        """
        if self._bunch is None:
    ...
    def __set_weight(self):
        """
        set weight for dataset, and this value
        will be added to self._bunch object.
        """
    ...
    def __auto_set_weight(self):
        """
        get weight column from auto mode.
        """
        ...
        if dataset_weight is None:
            if self._task_name == ConstantValues.binary_classification or ConstantValues.multiclass_classification:
                ...
                self._bunch.weight_names = list(self._bunch.dataset_weight.columns)
    ...
    def __load_csv(self):
        """
        load csv file from self._data_path.
        """
        if not isinstance(self._column_name_flag, bool):
        ...
        if self._column_name_flag:
            ...
            if self.__use_weight_flag:
                if self._weight_column_names:
                    ...
                    weight_names = list(weight.columns)
        ...
        return data, target, weight, feature_names, target_names, weight_names

    def __load_libsvm(self):
        """
        load libsvm file from self._data_path
        """
        if self._name == ConstantValues.train_dataset:
            if bool(self._weight_column_names) and bool(self.__dataset_weight_dict):
                message = "Just one weight setting can be set."
    ...
    def __load_txt(self):
        """
        load txt file from self._data_path.
        """
        if self._column_name_flag is True:
    ...
    def __wc_count(self):
        """
        calculate lines from dataset file.
        """
        import subprocess
    ...
    @classmethod
    def __convert_data_dataframe(cls, data, target,
                                 feature_names=None, target_names=None):
        """
        This method will convert dataset object to dataframe.
        """
    ...
    def split(self, val_start: float = 0.8):
        ...
        if self._bunch.dataset_weight is not None:
            ...
            data_package.weight_names = self._bunch.weight_names
        else:
            data_package.dataset_weight = None
            data_package.weight_names = None
