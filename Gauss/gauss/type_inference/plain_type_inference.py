...
from Gauss.gauss.type_inference.base_type_inference import BaseTypeInference
from Gauss.entity.feature_configuration.feature_config import FeatureItemConf, FeatureConf
        ...
        for index, column in enumerate(data.columns):
            ...
            if self.final_feature_configure.feature_dict[column].dtype == 'string':
                ...
                if data[column].isnull().any():
                    categorical_detect = (len(pd.unique(data[column])) - 1)/data[column].shape[0]
                else:
