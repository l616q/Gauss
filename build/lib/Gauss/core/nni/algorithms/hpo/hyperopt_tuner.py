...
from Gauss.core.core_tuning import hyperopt as hp
import numpy as np
from schema import Optional, Schema
from Gauss.core.nni import ClassArgsValidator
from Gauss.core.nni.tuner import Tuner
from Gauss.core.nni.utils import NodeType, OptimizeMode, extract_scalar_reward, split_index
...
def json2space(in_x, name=NodeType.ROOT):
    """
    Change json to search space in hyperopt_example.
...
def _add_index(in_x, parameter):
    """
    change parameters in NNI format to parameters in hyperopt_example format(This function also support nested dict.).
    For example, receive parameters like:
        {'dropout_rate': 0.8, 'conv_size': 3, 'hidden_size': 512}
    Will change to format in hyperopt_example, like:
...
class HyperoptTuner(Tuner):
    """
    HyperoptTuner is a tuner which using hyperopt_example algorithm.
    """
    ...
    @classmethod
    def _choose_tuner(cls, algorithm_name):
        """
        Parameters
        ----------
        algorithm_name : str
        ...
        raise RuntimeError('Not support tuner algorithm in hyperopt_example.')

    def update_search_space(self, search_space):
        """
        Update search space definition in tuner by search_space in parameters.

        Will called when first setup experiment or update search space in WebUI.
    ...
    def generate_parameters(self, parameter_id, **kwargs):
        ...
        # avoid generating same parameter with concurrent trials because hyperopt_example doesn't support parallel mode
    ...
    def receive_trial_result(self, parameter_id, parameters, value, **kwargs):
        ...
        # restore the parameters contains '_index'
        ...
        if self.parallel:
            ...
            if constant_liar:
                ...
                rval = self.rval
                # ignore duplicated reported final result (due to aware of intermediate result)
    ...
    def get_suggestion(self, random_search=False):
        """
        get suggestion from hyperopt_example
        ...
        if self.parallel and len(self.total_data) > 20 and self.running_data and self.optimal_y is not None:
            ...
            for _parameter_id in self.running_data:
                self.receive_trial_result(parameter_id=_parameter_id, parameters=params, value=_constant_liar_y,
                                          constant_liar=True)
            rval = self.CL_rval

            random_state = np.random.randint(2 ** 31 - 1)
        else:
            # rval: hp.FMinIter() object
            rval = self.rval
    ...
    def import_data(self, data):
        ...
        ----------
        data:
            a list of dictionary, each of which has at least two keys, 'parameter' and 'value'
        """
        _completed_num = 0
