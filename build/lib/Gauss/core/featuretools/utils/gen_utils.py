...
from Gauss.core.tqdm import tqdm
...
def check_schema_version(cls, cls_type):
    if isinstance(cls_type, str):
        if cls_type == 'entityset':
            from Gauss.core.featuretools.entityset.serialize import SCHEMA_VERSION
            version_string = cls.get('schema_version')
        elif cls_type == 'features':
            from core.featuretools.feature_base.features_serializer import (
                SCHEMA_VERSION
            )
